...
import { Session, ChartStats, ChartRange, UserGoal } from '../types';
...
export const HomeView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // 3. Load Stats & User Data if registered
  ...
  return (
    <div className="flex flex-col h-full p-6 space-y-8 overflow-y-auto pb-24">
      ...
      <header className="flex justify-between items-center mt-2">
        <div>
          <h1 className="text-h1">{greeting}, {userName || 'אורח'}</h1>
      ...
      {
        userGoal && (
          <div className="bg-neu-base rounded-2xl p-4 shadow-neu-flat flex items-center justify-between animate-in fade-in slide-in-from-top-4 duration-500">
            <div>
              <p className="text-meta text-xs mb-1">היעד שלי</p>
              <h3 className="text-h2">יום {chartStats.streak > 0 ? chartStats.streak : 1} מתוך {userGoal.targetDays}</h3>
              <div className="w-32 h-2 bg-neu-base shadow-neu-inner rounded-full mt-2 overflow-hidden">
                <div
                  className="h-full bg-neu-dark"
                  style={{ width: `${Math.min(100, ((chartStats.streak || 1) / userGoal.targetDays) * 100)}%` }}
                />
              </div>
            </div>
            <div className="w-12 h-12 rounded-full bg-neu-base shadow-neu-pressed flex items-center justify-center text-primary">
